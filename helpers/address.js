...
const getRaffleActorsAsync = async options => {
  const accounts = await getAccountsAsync();

  return getRaffleActors(accounts, options);
};
...
const getEthersStakingActor = async address => {
  const signers = await ethers.getSigners();

  return signers.find(a => a.address === address);
};

const getEthersStakingActors = () => ethers.getSigners();
...
module.exports = {
  ...
  getEthersStakingActor,
  getEthersStakingActors
};
